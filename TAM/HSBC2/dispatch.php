...
//var_dump($_POST);
$shift="";
$sqlshift="SELECT SHIFT FROM ENGINEER WHERE CEC='$engcec'";
...
if(isset($_POST["Edit"])){
    ...
    if($zone=="IST"){
        ...
        $toadd = new DateInterval('PT09H30M');
        $datedis->add($toadd);
        $dtdis=$datedis->format('Y-m-d H:i:s');
        //echo $dtdis;
    }
        ...
        $toadd = new DateInterval('PT02H30M');
        $datedis->add($toadd);
        $dtdis=$datedis->format('Y-m-d H:i:s');
        //echo $dtdis;
    }
    ...
    $today=date("Y-m-d");
    $sql = "INSERT INTO DISPATCH(CASE_NUM, ENG_CEC, CUSTOMER, DT_SUBMIT, SHIFT, DT_DISPATCH, TYPE,ASSIGNED) VALUES('$casenum','$engcec','$customer','$dtsubmit','$shift','$dtdis','$type',0);";
    //echo $sql;
...
<!-- <input type="datetime" required style="display:"> -->
					...
					<!-- <div class="field half tnb">
						...
						<input type="text" id="qID" required readonly />
					</div> -->
					<div class="field half tnb">
					...
					<div class="field half tnb last">
						...
						<input type="text" id="datetime" value="'.date('Y-m-d H:i:s').'" readonly required />
					...
					<input class="button-primary" type="submit" name="Edit" value="Submit">
