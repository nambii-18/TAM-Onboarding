...
<html lang='en'>
    ...
    <head>
        ...
        <meta charset='utf-8'>
        <title>Admin Dashboard - TAM Tool &middot; Cisco CMS</title>
    ...
    <path transform='translate(144.9274,727.8212)' d='M-286.3-357.7c-5.2,0-9.1,4.1-9.1,9.1c0,5.1,3.9,9.1,9.1,9.1s9.1-4.1,9.1-9.1S-281.1-357.7-286.3-357.7 M-267.9-348.5 c0,9.9-7.7,18-18.4,18s-18.4-8.1-18.4-18s7.7-18,18.4-18S-267.9-358.4-267.9-348.5'/>
      ...
      <div class='row' style='margin-top:2%'>
        <div class='five columns'>
                ...
                <a href='javascript:;' id='' class='button' style='margin-bottom: 2rem;display:inline-block;margin-top: -1px;border: 0; border-bottom: 1px solid #212121; padding: 0; margin-right:20px;'>Case Statistics Dashboard</a>

                        <a href='#' id='' class='button' style='margin-bottom: 2rem;display:inline-block;margin-top: -1px;border: 0; border-bottom: 1px solid #212121; padding: 0;' role='button'>Review Results Dashboard</a>


                        <h5 style='font-size: 15px;font-weight: 600;letter-spacing: .1rem;text-transform: uppercase;text-align:right;'></br></h5>


                        <h5 style='font-size: 15px;font-weight: 600;letter-spacing: .1rem;text-transform: uppercase;text-align:right;'></br></h5>

                        <h5 title='Click to Expand' style='font-size: 15px;font-weight: 600;letter-spacing: .1rem;text-transform: uppercase;text-align:right;'></br></h5>

                </span>

                <table id='finesse' style='width: 100%;'>
                              <thead>
                                 <tr>
                                  <th>Agent Extension</th>
                                  <th>Finesse State</th>
                                  <th>Time in Current State (hh:mm)</th>
                                  <th>Skill Group (SG)</th>
                                  <th>CEC</th>
                                </tr>
                              </thead>
                              <tbody id='sortthis2'>
...
//function awyeah() {
    ...
    {
      var arr=[];
      var tr_array = document.getElementById("sortthis").getElementsByTagName("tr");
      console.log(tr_array);
      for(var i=0;i<tr_array.length-1;i++){
      var t= (tr_array[i].getElementsByTagName("td")[0].innerHTML);
      arr.push(t);
      }
      console.log(arr);
       json = JSON.stringify(arr);
      console.log("pop"+json);
      ob["map"]= '<?php echo json_encode($mp);?>';
      ob["engarr"]= json;
      console.log(json+"data");
}
updateEng();
 setInterval(updateEng,30000);
...
//function awyeah() {
        ...
        $.ajax({
          ...
          //  console.log("earr"+'<?php echo json_encode($earr);?>');

          }
        });

        setTimeout (finesselogin, 30000);
        //setInterval(awyeah, 30000 );
      });
